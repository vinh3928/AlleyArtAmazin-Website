  ...
  {  	
    if( $_POST['action'] == 'export_csv' || $_POST['action'] == 'export_xml' || $_POST['action'] == 'export_excel' || $_POST['action'] == 'export_json' )
      ...
      if(isset($_POST['location']))
  		...
  		{
  			  $query = $wpdb->prepare("SELECT * FROM ".$location_table." where %s ","1=1");
...
class Wpgmp_Location_Table extends WP_List_Table {
    ...
    global $status, $page,$wpdb;
		...
		if($_GET['page']=='wpgmp_manage_location' && !empty($_REQUEST['s']) )
		{
		  $query = "SELECT * FROM ".$wpdb->prefix."map_locations WHERE location_title LIKE '%".$_REQUEST['s']."%' OR location_address LIKE '%".$_REQUEST['s']."%' OR location_latitude LIKE '%".$_REQUEST['s']."%' OR location_longitude LIKE '%".$_REQUEST['s']."%'";
		}
		else
		{
...
<form method="post">
  ...
  <div class="row">      	
  <div class="col-md-3">    <label for="Description"><?php _e('Address', 'wpgmp_google_map')?>&nbsp;</label></div>
  <div class="col-md-7">
  ...
  <p class="description"> <?php _e('Enter here the longitude.', 'wpgmp_google_map')?></p></div>
   ...
   <?php _e('Do you want to disable infowindow for this location?', 'wpgmp_google_map')?></p></div>
    ...
    <div class="row">
    <div class="col-md-3">    <label for="Image"><?php _e('Choose Marker Image', 'wpgmp_google_map')?></label></div>
   <div class="col-md-7"> 
