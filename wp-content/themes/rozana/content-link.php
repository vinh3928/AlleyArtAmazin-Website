...
if ( $blog_type == 'grid' && ! is_single() ) {
	// Blog Grid
	get_template_part( 'blog-templates/content-link', 'grid' );
} elseif ( $blog_type == 'list' && ! is_single() ) {
	// Blog type list
	get_template_part( 'blog-templates/content-link', 'list' );
} elseif ( $blog_type == 'masonry' && ! is_single() ) {
	// Blog masonry
	get_template_part( 'blog-templates/content-link', 'masonry' );
} elseif ( ( $blog_type == 'bigthumbnails' || $blog_type == 'bigthumbwithsidebar' ) && ! is_single() ) {
	// Blog Big Thumbnails
	get_template_part( 'blog-templates/content-link-big', 'thumb' );
} elseif ( ( $blog_type == 'wpdefalutfullwidth' || $blog_type == 'wpdefaultwithsidebar' ) && ! is_single() ) {
	// Blog Wordpress Default
	get_template_part( 'blog-templates/content-link-wp', 'default' );
} elseif( is_single() ) {
?>
	<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
		<?php if ( has_post_thumbnail() && isset( $sama_options['single_standard_at_top'] ) && $sama_options['single_link_at_top'] == 'thumb' ) { // check if the post has a Post Thumbnail assigned to it. ?>
			<!-- Banner Image -->
